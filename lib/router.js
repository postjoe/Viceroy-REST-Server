...
Router.prototype.resource = function(resourceName, opts, subRouteLoader) {
  ...
  if(opts.middleware && ['object' ,'function'].indexOf(typeof opts.middleware) > -1) {
  ...
  this._contextMiddleware = opts.middleware;
  delete opts.middleware;

  // get the id token
  var idToken = this._getId(resourceName);

  // setup the context
  var lastContextUrl = this._contextUrl;
  if(this._contextId) {
    this._contextUrl += '/' + this._contextId;
  }
  this._context.push({
    resourceName: resourceName,
    idToken: idToken.substr(1)
  });

  var resourceUri = tools.hyphenate(resourceName);

  // setup each route for the resource.
  if(actions.index) {
    opts.action = 'index';
    this._bind('get', resourceUri, opts);
  }
  if(actions.show) {
    opts.action = 'show';
    this._bind('get', resourceUri + '/' + idToken, opts);
  }
  if(actions.create) {
    opts.action = 'create';
    this._bind('post', resourceUri, opts);
  }
  if(actions.update) {
    opts.action = 'update';
    this._bind('put', resourceUri + '/' + idToken, opts);
    this._bind('patch', resourceUri + '/' + idToken, opts);
  }
  if(actions.destroy) {
    opts.action = 'destroy';
    this._bind('delete', resourceUri, opts);
    this._bind('delete', resourceUri + '/' + idToken, opts);
  }

  // restore the context url.
  ...
  if(subRouteLoader) {
    ...
    // set the context.
    var lastContextMiddleware = this._contextMiddleware;
    var lastContextUrl = this._contextUrl;
    this._contextUrl = '/' + resourceUri;
    this._contextId = this._getSubId(resourceName);
    this._context.push({
      resourceName: resourceName,
      idToken: this._contextId.substr(1)
    });
    ...
    this._contextId = '';
    this._contextUrl = lastContextUrl;
    this._contextMiddleware = lastContextMiddleware;
  }
...
Router.prototype._bind = function(method, routeUrl, opts, subRouteLoader) {
  ...
  if(
    opts.middleware &&
    opts.middleware.constructor != Array
  ) { opts.middleware = [opts.middleware]; }
  ...
  // grab the middleware.
  var middleware;
  if(this._contextMiddleware.length > 0) {
    middleware = this._filterMiddleware(opts.action, this._contextMiddleware);
  } else {
    middleware = [];
  }
  if(opts.middleware) {
    for(var i = 0; i < opts.middleware.length; i += 1) {
      middleware.push(opts.middleware[i]);
    }
  }

  // ensure the route url has a trailing slash and
  // get the full route including the context.
...
Router.prototype._getId = function(resourceName) {
  return ':' + this.tokens.id.replace('!{resourceName}', lingo.en.singularize(resourceName));
};

Router.prototype._getSubId = function(resourceName) {
