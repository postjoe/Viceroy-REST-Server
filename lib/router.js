...
function Router(server) {
  ...
  if(typeof server.resourceIdSufix != 'string') {
    throw new Error('server.resourceIdSufix must be a string');
  }
  ...
  this.resourceIdSufix = server.resourceIdSufix;
  this.state = 'ready';
  this._routes = {};
...
Router.prototype.resource = function(resourceName, opts, subRouteLoader) {
  ...
  if(subRouteLoader) {
    ...
    this._contextUrl = '/' + resourceName + '/' + this._createIdToken(resourceName);
...
Router.prototype._createIdToken = function(resourceName) {
  return ':' + this.resourceIdPrefix + lingo.en.singularize(resourceName) + this.resourceIdSufix;
};
