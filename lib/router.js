...
Router.prototype.resource = function(resourceName, opts, subRouteLoader) {
  ...
  if(actions.show) {
    opts.action = 'show';
    this._bind('get', resourceName + '/' + idToken, opts);
  }
  if(actions.create) {
    opts.action = 'create';
  ...
  if(actions.update) {
    opts.action = 'update';
    this._bind('put', resourceName + '/' + idToken, opts);
    this._bind('patch', resourceName + '/' + idToken, opts);
  }
  if(actions.destroy) {
    opts.action = 'destroy';
    this._bind('delete', resourceName, opts);
    this._bind('delete', resourceName + '/' + idToken, opts);
  }
  ...
  if(subRouteLoader) {
    ...
    // clear the context id and reset the context
    // url.
    this._contextId = '';
    this._contextUrl = lastContextUrl;
