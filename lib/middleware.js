...
Middleware.prototype._handleRequest = function(req, res, next) {
  ...
  baseController.handleRequest(this._models, req, res, function(err) {
    if(err) { return next(err); }

    var execAction = function(cb) {
    ...
    var execMiddleware = function(cb) {
      if(route.middleware.length > 0) {
        ...
        (function exec() {
          route.middleware[i](req, res, function(err) {
            ...
            i += 1;
            if(i < route.middleware.length) { exec(); }
            else { cb(); }
          });
        })();
      } else {
    ...
    // run the middleware and
    execMiddleware(function(err) {
      if(err) { return next(err); }
      execAction(next);
    });
  });
